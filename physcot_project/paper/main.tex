...
% ─── Title ────────────────────────────────────────────────────────────────────
\icmltitlerunning{PhysCoT: Physics-Intuitive Chain-of-Thought Prompting for Robot Manipulation}

\begin{document}

\twocolumn[
\icmltitle{PhysCoT: Physics-Intuitive Chain-of-Thought Prompting\\
           for Robot Manipulation with OpenVLA}

\icmlsetsymbol{equal}{*}

\begin{icmlauthorlist}
\icmlauthor{Bobby Shi}{su}
\end{icmlauthorlist}

\icmlaffiliation{su}{Department of Computer Science, Stanford University, Stanford, CA, USA}

\icmlcorrespondingauthor{Bobby Shi}{shi02@stanford.edu}

\icmlkeywords{robot manipulation, chain-of-thought reasoning, vision-language-action models,
              physics-intuitive reasoning, prompt engineering}
...
a \emph{prompt-only} inference-time wrapper that augments any VLA policy with
four structured reasoning stages—task decomposition, relevant physics identification,
approximate visual physical estimation, and physics-aware action derivation—without
...
roadmap for converting the prompt-only approach into a supervised training pipeline.
...
However, ECoT training is expensive: it requires a dataset of (observation, reasoning, action)
triples and a finetuning step on top of the base VLA.
Moreover, the reasoning content in ECoT is largely \emph{generic}—it does not
...
We ask whether explicit, structured \emph{physics-intuitive} reasoning
can be added at \emph{inference time}—without any training—by designing
the right prompt scaffold.
...
\begin{itemize}
  \item We introduce \physcot{}, the first \emph{prompt-only} physics-intuitive
...
at inference time (no training), and $\pi$ is a frozen \openvla{} checkpoint.
...
We implement a pure-Python 2D rigid-body physics simulator using NumPy,
...
\physcot{} is prompt-only: it cannot update the policy weights.
While this makes the approach accessible, a trained model with
internalized physics reasoning would likely outperform prompt-only inference
...
Our 2D rigid-body simulator captures the physics relevant to our tasks
but excludes 3D effects, deformable objects, complex contact dynamics,
and real-robot actuation noise.
...
The most promising direction is to convert the prompt-only approach
...
Our experiments demonstrate that structured physics reasoning—even at inference time
without any training—yields consistent, interpretable improvements over a plain
\openvla{} baseline on tasks that specifically require physical intuition.

The gains are real but bounded by the prompt-only constraint and small trial counts.
...
Action: push block laterally at {contact_frac}
  of height.
\end{verbatim}

\subsection{PhysCoT Block Toppling Prompt}
\begin{verbatim}
Task decomposition:
...
Relevant physics:
  ...
  Topple if y_c > mgw/(2F) = y_c*
  Friction: F_slide = μmg

Visual physical estimates:
  Aspect ratio h/w = {aspect}
  Best contact zone: [{lo}, 0.88] × h

Action implication:
  Contact at {y_c}% height (above y_c*)
  Lateral horizontal push.
  τ_push = {tau} > τ_crit: topple expected.
\end{verbatim}

\subsection{PhysCoT Tool Selection Prompt}
\begin{verbatim}
Task decomposition:
...
Visual physical estimates:
  Direct path: {BLOCKED | CLEAR}
  Obstacle at ({obs_x}, {obs_y}), size {w}×{h}

Action implication:
  Select {HOOK | STRAIGHT}.
  Approach from open lateral side.
\end{verbatim}

\section{Full Trial Logs}
\label{app:logs}

Full trial-by-trial logs (40 trials) are available in
\texttt{results/metrics/all\_trials.json} in the project repository.
Each log entry contains all fields listed in Section~\ref{sec:setup},
including the full reasoning text for every \physcot{} trial.

\section{Video Materials}
\label{app:videos}

MP4 videos for all 40 trials are stored in \texttt{results/videos/}.
\begin{itemize}
  \item \texttt{exp1\_block\_toppling/baseline/trial\_XX.mp4}
  \item \texttt{exp1\_block\_toppling/physcot/trial\_XX.mp4}
  \item \texttt{exp2\_tool\_selection/baseline/trial\_XX.mp4}
  \item \texttt{exp2\_tool\_selection/physcot/trial\_XX.mp4}
\end{itemize}
